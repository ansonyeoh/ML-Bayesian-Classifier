...
for c = 1:length(cl)
    ...
    Probs(:,:,c) = reshape(const_test*exp(-0.5*diag(temp*inv(sigmac)*temp')),size(Xv));
...
for i =1:2
    ...
    if i == 1        
        ti = sprintf('ML with Naive Probability for class diseased');
    else 
        ti = sprintf('ML with Naive Probability for class healthy');
    end
    title(ti);
...
for i =1:2
    ...
    contour(Xv,Yv,Probs(:,:,i));
    if i == 1        
        ti = sprintf('ML without Naive Probability for class diseased');
    else 
        ti = sprintf('ML without Naive Probability for class healthy');
    end
    title(ti);
    legend('Diseased','Healthy');
end